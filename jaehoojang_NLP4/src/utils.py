...
TOKENIZER_CLASSES = {
    ...
    "kcelectra-base": ElectraTokenizer,
...
MODEL_FOR_SEQUENCE_CLASSIFICATION = {
    ...
    "kcelectra-base": ElectraForSequenceClassification,
}
...
def acc_f1_pre_rec(labels, preds, is_ner=True):
    if is_ner:
        return {
            "acc": simple_accuracy(labels, preds),
            "precision": seqeval_metrics.precision_score(labels, preds, suffix=True),
...
def compute_metrics(task_name, labels, preds): #metric 결정
    ...
    if task_name == "kornli":
        ...
        return acc_f1_pre_rec(labels, preds, is_ner=False)
    elif task_name == "ISDS":
        return acc_f1_pre_rec(labels, preds, is_ner=False)
    elif task_name == 'EMPD':
        return acc_f1_pre_rec(labels, preds, is_ner=False)
