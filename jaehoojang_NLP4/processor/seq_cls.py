...
class EmotionProcessor(object):
    """Processor for the Korean Emotion data set """
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines[1:]):
            ...
            if i % 1000 == 0:
                logger.info(line)
            examples.append(InputExample(guid=guid, text_a=text, text_b=None, label=label)) #InputExample 메소드에서 tensor 생성
...
class ISDSProcessor(object):
    ...
    def get_labels(self):
        return ["협박", "해당없음", "갈취공갈", "직장내괴롭힘", "기타괴롭힘"]
...
class EMPDProcessor(object):
    """Processor for the EMPD set """

    def __init__(self, args):
        self.args = args

    def get_labels(self):
        return ['surprised', 'anticipating', 'devastated', 'disappointed', 'angry', 'apprehensive', 'afraid', 'sentimental', 'terrified', 'disgusted', 'prepared', 'jealous', 'sad', 'embarrassed', 'confident', 'caring', 'annoyed', 'guilty', 'faithful', 'content', 'grateful', 'hopeful', 'lonely', 'joyful', 'nostalgic', 'anxious', 'trusting', 'impressed', 'proud', 'furious', 'ashamed', 'excited']
...
seq_cls_tasks_num_labels = {"kornli": 3, "nsmc": 2, "paws": 2, "korsts": 1, "question-pair": 2, "hate-speech": 3, "emotion" : 4, "ISDS" : 5, 'EMPD' : 32}
...
############### length 추출을 위한 추가 부분
    ...
    batch_dump = tokenizer.batch_encode_plus(
        [(example.text_a, example.text_b) for example in examples],
        add_special_tokens=True,
        truncation=True,
    )
    ...
    for i in range(len(examples)):
        input_dump = {k: batch_dump[k][i] for k in batch_dump}
        if "token_type_ids" not in input_dump:
            input_dump["token_type_ids"] = [0] * len(input_dump["input_ids"])  # For xlm-roberta

        feature_d = InputFeatures(**input_dump, label=labels[i])
        feature_dump.append(feature_d)
#
    for j, example in enumerate(examples):
        seq_len.append(len(feature_dump[j].input_ids))  #feature_dump[j]는 딕셔너리, value = list 반환

    print('seq_len 개수 : {} 개'.format(len(seq_len)))
...
def seq_cls_load_and_cache_examples(args, tokenizer, mode):
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}_{}".format( str(args.task), list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_len), mode),)

    cached_dump_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}_{}_".format(str(args.task), list(filter(None, args.model_name_or_path.split("/"))).pop(),
                                    str(args.max_seq_len), mode), )

    if os.path.exists(cached_features_file) and os.path.exists(cached_dump_file):
        logger.info("Loading features from cached file %s", cached_features_file)

        features = torch.load(cached_features_file)
        seq_len = torch.load(cached_dump_file)
        ...
        features, seq_len = seq_cls_convert_examples_to_features(
            args, examples, tokenizer, max_length=args.max_seq_len, task=args.task
        )
        logger.info("Saving features into cached file %s", cached_features_file)
        torch.save(features, cached_features_file)
        torch.save(seq_len, cached_dump_file)

    # Convert to Tensors and build dataset
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
    all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long)
    all_seq_length = torch.tensor(seq_len, dtype=torch.long)

    if output_mode == "classification":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
        # print('label 텐서 사이즈 : ', all_labels.size())
        # print('seq_length 텐서 사이즈 : ', all_seq_length.size())
    elif output_mode == "regression":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.float)

    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_seq_length)
    # for i in features:
    #     print(type(i))
    return dataset
