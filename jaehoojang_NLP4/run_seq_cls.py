...
def train(args,
    ...
    # for name, para in model.named_parameters(): #parameter확인
    ...
    #     print('파라미터 확인 :',name, para.requires_grad)
    ...
    for epoch in mb:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "labels": batch[3],
                "seq_length" : batch[4] #모델의 forward() 내 argument 이름과 맞춰 줌
            }

            if args.model_type not in ["distilkobert", "xlm-roberta","distilbert"]:
                inputs["token_type_ids"] = batch[2]  # Distilkobert, XLM-Roberta don't use segment_ids

            #print('input입니다 확인하세요 :', inputs)

            outputs = model(**inputs) #딕셔너리의 키가 argument 이름과 같아야 함.
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    with open(os.path.join(output_dir, 'config.json'), 'w') as f:
                        json.dump(conf_json, f, ensure_ascii=False, indent='\t')
...
def evaluate(args, model, eval_dataset, mode, global_step=None):
    ...
    for batch in progress_bar(eval_dataloader):
        ...
        if preds is None:
            ...
            #print(logits.detach().cpu().numpy().shape) #마지막 배치 처리 modeling_electra에서 squeeze빼버림.
    ...
    # for i ,j in zip(out_label_ids, preds):
...
def main(cli_args):
    ...
    if output_modes[args.task] == "regression":
        ...
        config = CONFIG_CLASSES[args.model_type].from_pretrained(
            ...
            MLP_temp=args.MLP_temp,
            seq_length=args.seq_length,
            attention_probs_dropout_prob=args.attention_probs_dropout_prob,
            hidden_dropout_prob=args.hidden_dropout_prob,
            early_stopping=args.early_stopping
        )
        print('config 확인 : ', config.label2id)
